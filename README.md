...
- Prefer explicit domain namespaces for advanced features: `oodocs.structure` for appendices and theorem-like blocks; `oodocs.media` for advanced media/table helpers; `oodocs.references` for advanced reference formatting helpers; `oodocs.engineering` for algorithms and engineering equation classes; `oodocs.chemistry` for chemistry notation; `oodocs.glossary` for glossary/acronym registries; `oodocs.review` for TODOs and margin notes; `oodocs.positioning` for page overlays; and `oodocs.generated` for generated list pages.
