...
- Import `ColumnSpec(...)` from `oodocs.media` for fixed columns and `tabularx`-style flex columns in `Table(...)`, `Table.from_dataframe(...)`, `Table.from_csv(...)`, and `Table.from_tsv(...)`. In `Table.from_records(...)`, `ColumnSpec(key=..., header=..., visible=False)` can also choose, rename, or hide record fields.
