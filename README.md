...
- LaTeX `glossaries` / `acronym` / `nomencl` -> `Glossary(...)`, `glossary.term(...)`, `glossary.acronym(...)`, `glossary.use(...)`, `ListOfGlossaryTerms(...)`, plus `Nomenclature`
- LaTeX `babel` / `polyglossia` document language -> `Theme.from_locale("ko-KR")` or `LocaleDefaults.from_locale(...)` for localized labels, generated titles, dates, HTML `lang`, and PDF font guidance
...
- LaTeX `mhchem` chemistry notation -> `chemical_formula(...)`, `ce(...)`, and `ReactionEquation(...)` from `oodocs.chemistry`, covering formula subscripts, charge superscripts, Unicode script input, and reaction references
- LaTeX `natbib` / `biblatex` / BibTeX citations -> `CitationLibrary.from_bibtex_file(...)`, `cite(...)` or `CitationSource.cite()`, `CitationDefaults(...)`, and `ListOfReferences(...)`
- LaTeX `fancyhdr` / `scrlayer-scrpage` running headers and footers -> `Theme(header_footer=HeaderFooterDefaults(...))` with `{page}`, `{title}`, `{chapter}`, `{section}`, first-page, and even-page templates
...
- Use `link(target, label)` for named external links and `url(target, breakable=True)` when the visible text is the URL itself. Long raw URL labels emit a validation warning because DOCX, PDF, and HTML wrap them differently; see [docs/reference/hyperref-support.md](docs/reference/hyperref-support.md) for anchors, metadata, PDF outline entries, link styling, and broken-link validation.
...
- Import `ColumnSpec(...)` from `oodocs.media` for fixed columns, `tabularx`-style flex columns, column alignment, and column cell styling in `Table(...)`, `Table.from_dataframe(...)`, `Table.from_csv(...)`, and `Table.from_tsv(...)`. In `Table.from_records(...)`, `ColumnSpec(key=..., header=..., visible=False)` can also choose, rename, or hide record fields.
...
- Use `CodeBlock(..., caption=..., line_numbers=True, highlight_lines={...})` for numbered listings, `ListOfListings()` from `oodocs.generated` for a generated listing index, and `CodeBlock.from_file("example.py", caption=...)` when the code should be included from a source file.
...
- Use `Theme.from_locale("ko-KR")` or `Theme(locale=LocaleDefaults.from_locale("ko-KR"))` for a bundled document language: localized captions, generated page titles, glossary labels, reference-list title, date formatting with `theme.format_date(...)`, HTML `lang`, and PDF font guidance via `theme.pdf_font_fallback_guide()`; see [docs/reference/locale-support.md](docs/reference/locale-support.md).
- Use `CitationLibrary.from_bibtex_file("refs.bib")` for BibTeX input, and `Theme(citations=CitationDefaults(citation_style="apa", reference_style="apa", reference_sort="author"))` when inline citations and the generated references page should follow an author-year style. Numeric citation output and citation-order references remain the default; pass `ListOfReferences(include_uncited=True)` when the generated bibliography should include uncited library entries. See [docs/reference/citation-support.md](docs/reference/citation-support.md) for the supported BibTeX subset, sort styles, naming policy, and CSL boundary.
- Import `Glossary(...)` and `ListOfGlossaryTerms(...)` from `oodocs.glossary` for collected terminology. `glossary.use("API")` expands acronyms on first use, and `ListOfGlossaryTerms(glossary)` renders the generated glossary table; see [docs/reference/glossary-support.md](docs/reference/glossary-support.md).
- Use `footnote("term", "note", stream="symbols")` with `Theme(footnotes=FootnoteDefaults(stream_styles={"symbols": FootnoteStyle.symbol()}))` when notes need independent numeric or symbol streams. Plain default footnotes still use native DOCX page footnotes; custom streams fall back to the generated notes page in DOCX. Use `ListOfFootnotes()` from `oodocs.generated` when the generated notes page should appear at a specific location; see [docs/reference/footnote-support.md](docs/reference/footnote-support.md).
- Import `todo(...)`, `Todo(...)`, `margin_note(...)`, and `MarginNote(...)` from `oodocs.review` for review tasks and side notes that should stay next to the source prose. `MarginNote(...)` renders as an HTML side note, while DOCX and PDF keep the note through comment-style fallback output; see [docs/reference/review-annotation-support.md](docs/reference/review-annotation-support.md).
...
- Use `Theme(header_footer=HeaderFooterDefaults(header_left="{chapter}", header_right="{page}", footer_center="{title}", different_first_page=True))` for LaTeX `fancyhdr`-style running headers and footers. `{page}`, `{title}`, `{chapter}`, and `{section}` are supported; DOCX uses Word fields, PDF draws them in the page callback, and HTML emits a sticky/fixed degrade layer. See [docs/reference/header-footer-support.md](docs/reference/header-footer-support.md).
...
- portable comments, TODO annotations, margin notes, and footnotes that stay stable across DOCX, PDF, and HTML; see [docs/reference/review-annotation-support.md](docs/reference/review-annotation-support.md) for review-only helpers
- footnotes target page-bottom placement by default when the renderer supports it; `Theme(blocks=BlockDefaults(footnote_placement="document"))` keeps the collected-notes pattern, and `FootnoteDefaults` supports independent numeric or symbol footnote streams; see [docs/reference/footnote-support.md](docs/reference/footnote-support.md)
