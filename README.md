...
- Use `subscript(...)`, `superscript(...)`, `prescript(...)`, and `inline_math(...)` for ordinary prose. Import `AlignedEquation(...)` and `CasesEquation(...)` from `oodocs.engineering` for lightweight LaTeX-style aligned derivations and piecewise cases. Use `Equation(...)` for displayed expressions, including ordinary `x^2` / `x_0` scripts and front scripts such as `\prescript{14}{6}{C}`. Pass `numbered=False` when a displayed equation should not consume an equation number.
...
- Prefer explicit domain namespaces for advanced features: `oodocs.structure` for appendices and theorem-like blocks; `oodocs.media` for advanced media/table helpers; `oodocs.references` for advanced reference formatting helpers; `oodocs.engineering` for algorithms, aligned/cases equations, and chemistry; `oodocs.glossary` for glossary/acronym registries; `oodocs.review` for TODOs and margin notes; `oodocs.positioning` for page overlays; and `oodocs.generated` for generated list pages.
