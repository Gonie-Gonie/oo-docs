...
- Prefer explicit domain namespaces for advanced features: `oodocs.structure` for appendices and theorem-like blocks; `oodocs.media` for advanced media/table helpers; `oodocs.engineering` for algorithms, aligned/cases equations, and chemistry; `oodocs.glossary` for glossary/acronym registries; `oodocs.review` for TODOs and margin notes; `oodocs.positioning` for page overlays; and `oodocs.generated` for generated list pages.
