...
- Use `ref(obj)` or `obj.ref()` for cross-references to captioned media, headings, equations, paragraphs, code blocks, and boxes. Use top-level `refs([a, b])` for plural object references and `ref_range(a, b)` for ranges. Import `ReferenceFormat`, `paren_ref`, and `page_ref` from `oodocs.references` when custom formatting, parenthesized references, or page-aware reference requests are needed. Passing the raw object inside `Paragraph(...)` is rejected so insertion and citation are not confused.
