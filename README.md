...
- Import `Shape(...)`, `TextBox(...)`, `ImageBox(...)`, and `PageItemScope` from `oodocs.positioning` for page-positioned overlays that do not move the body text. Use them with `DocumentSettings(page_items=[...])`; PDF applies scopes to physical pages, while DOCX and HTML use section/static-frame fallbacks. Use `placement="inline"` when the same objects should sit in the text flow like Word's inline drawing mode.
- Prefer explicit domain namespaces for advanced features: `oodocs.engineering` for algorithms, aligned/cases equations, and chemistry; `oodocs.glossary` for glossary/acronym registries; `oodocs.review` for TODOs and margin notes; `oodocs.positioning` for page overlays; and `oodocs.generated` for generated list pages.
