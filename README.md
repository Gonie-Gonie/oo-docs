...
- Use `parse_markdown(...)` or `parse_markdown_file(...)` when you need an `ImportResult` with import issues. Use `import_policy="fail-on-lossy"` when lossy Markdown features such as remote images should fail instead of being converted conservatively.
